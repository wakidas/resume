...
### textlintルール
- **MDXファイルサポート**: textlint-plugin-mdxを使用してMDXファイルもlint対象に含める
- **英単語スペースルール**: 日本語文章中の英単語の両端には半角スペースを入れる（例: 「CREグループ」→「CRE グループ」）
- **lintスクリプト**: `npm run lint`ですべてのMarkdownとMDXファイルをチェック
